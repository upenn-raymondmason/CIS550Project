...
const createGoogleUser = async (email, username, googleid) => {
  ...
  const response = await fetch('https://golazo-server.herokuapp.com/create_google_user/', requestOptions);
  return response.json();
};
...
const createFBUser = async (email, username, facebookid) => {
  ...
  const response = await fetch('https://golazo-server.herokuapp.com/create_fb_user/', requestOptions);
  return response.json();
};
